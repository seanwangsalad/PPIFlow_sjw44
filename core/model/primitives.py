...
from core.utils.checkpointing import get_checkpoint_fn, deepspeed_is_installed
...
def _deepspeed_evo_attn(
    ...
    # primitives.py
    # xk
    if orig_dtype not in [torch.bfloat16, torch.float16] or biases[0] not in [torch.bfloat16, torch.float16]:
