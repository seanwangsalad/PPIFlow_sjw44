...
def parse_chain_feats(chain_feats, scale_factor=1., normalize_positions=True):
    ca_idx = residue_constants.atom_order['CA']
    chain_feats['bb_mask'] = chain_feats['atom_mask'][:, ca_idx]#(180, 37)-->(180,)
    if normalize_positions:
        bb_pos = chain_feats['atom_positions'][:, ca_idx]#(180, 37, 3)-->(180, 3)
        bb_center = np.sum(bb_pos, axis=0) / (np.sum(chain_feats['bb_mask']) + 1e-5)#(3,)
        centered_pos = chain_feats['atom_positions'] - bb_center[None, None, :]#(180, 37, 3)
    else:
        centered_pos = chain_feats['atom_positions']
    scaled_pos = centered_pos / scale_factor
    chain_feats['atom_positions'] = scaled_pos * chain_feats['atom_mask'][..., None]
    chain_feats['bb_positions'] = chain_feats['atom_positions'][:, ca_idx]# (180, 3)
    return chain_feats
...
def parse_pdb_feats(
    ...
    def _process_chain_id(x):
        ...
        return parse_chain_feats(
            feat_dict, scale_factor=scale_factor, normalize_positions=False)
