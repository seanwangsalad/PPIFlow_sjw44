...
class Interpolant:
    ...
    def corrupt_batch(self, batch, cur_epoch):
        ...
        if self._trans_cfg.corrupt:
            mask = batch['pos_fixed_mask'] if 'pos_fixed_mask' in batch else batch['diffuse_mask']
            trans_t = self._corrupt_trans(
                trans_1, r3_t, res_mask, diffuse_mask=mask, trans_scale=self._trans_cfg.var_scale)
        else:
            trans_t = trans_1
    ...
    def sample(
        ...
        batch['cdr_mask'] = (1 - batch['binder_motif_mask']) * batch['pos_fixed_mask']
        ...
        if diffuse_mask is not None and trans_1 is not None and rotmats_1 is not None:
            ...
            if len(motif_mask.shape) == 1:
                motif_mask = motif_mask[None].expand((num_batch, -1))  # torch.Size([248])-->torch.Size([1, 248])
        else:
            motif_mask = None

        if motif_or_target:
            # multimotif: just fixed target pos, #else: fixed target+motif pos
            mask = batch['pos_fixed_mask'] if 'pos_fixed_mask' in batch else batch['diffuse_mask']
        ...
        for i, t_2 in enumerate(ts[1:]):
            ...
            if self._cfg.self_condition:
                if motif_or_target:
                    mask = batch['pos_fixed_mask'] if 'pos_fixed_mask' in batch else batch['diffuse_mask'] # multimotif: just fixed target pos, #else: fixed target+motif pos
                    batch['trans_sc'] = (
                        pred_trans_1 * mask[..., None]
                        + trans_1 * (1 - mask[..., None])
                    )
                else:
            ...
            if motif_or_target:
                mask = batch['pos_fixed_mask'] if 'pos_fixed_mask' in batch else batch['diffuse_mask']
                trans_t_2 = _trans_diffuse_mask(trans_t_2, trans_1, mask)
                rotmats_t_2 = _rots_diffuse_mask(rotmats_t_2, rotmats_1, mask)
        ...
        if trans_1 is not None and rotmats_1 is not None:
            mask = batch['pos_fixed_mask'] if 'pos_fixed_mask' in batch else batch['diffuse_mask']
            pred_trans_1 = _trans_diffuse_mask(pred_trans_1, trans_1, mask)
            pred_rotmats_1 = _rots_diffuse_mask(pred_rotmats_1, rotmats_1, mask)
    ...
    def sample_antibody(
        ...
        if binder_motif_mask is not None:
            batch['binder_motif_mask'] = binder_motif_mask
        if pos_fixed_mask is not None:
            batch['pos_fixed_mask'] = pos_fixed_mask
