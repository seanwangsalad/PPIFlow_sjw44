...
class Interpolant:
    ...
    def sample_t(self, num_batch, x=0, y=1):  # *
        t = torch.rand(1, device=self._device).expand(num_batch)

        # scale the range from [0, 1] to [x, y]
        t_scaled = t * (y - x) + x
        t_final = t_scaled * (1 - 2 * 0.01) + 0.01  # *
    ...
    def corrupt_batch(self, batch):
        ...
        if self._cfg.t_method == 'uniform':
            t = self.sample_t(num_batch, x=self._cfg.min_t, y=self._cfg.max_t)[:, None]
        ...
        if self._trans_cfg.corrupt:
            trans_t = self._corrupt_trans(
                ...
                init_binder_offsets=hotspot_center) #partial  # * shift binder diffuse center to hotspot center
        else:
            trans_t = trans_1
        ...
        if self._rots_cfg.corrupt:
            rotmats_t = self._corrupt_rotmats(
                rotmats_1, so3_t, res_mask, diffuse_mask) #partial
        else:
            rotmats_t = rotmats_1
    ...
    def sample(
        ...
        if trans_0 is None:
            ...
            #     num_batch, num_res, self._device) * du.NM_TO_ANG_SCALE#torch.Size([1, 70, 3])
            trans_0 = _centered_gaussian(
        ...
        if rotmats_0 is None:
            rotmats_0 = _uniform_so3(num_batch, num_res, self._device)#torch.Size([1, 70, 3, 3])
        if res_idx is None:
        ...
        if aatype_rc is not None:
            batch['aatype_rc'] = aatype_rc
        if trans_rc is not None:
            batch['trans_rc'] = trans_rc
        if rotmats_rc is not None:
            batch['rotmats_rc'] = rotmats_rc
        if rc_node_mask is not None:
            batch['rc_node_mask'] = rc_node_mask
        if motif_groups_mask is not None:
            batch['motif_groups_mask'] = motif_groups_mask

        motif_or_target = False
        if diffuse_mask is not None and trans_1 is not None and rotmats_1 is not None:
            motif_or_target = True
            motif_mask = ~diffuse_mask.bool().squeeze(0)
        else:
            motif_mask = None
        if motif_or_target and not self._cfg.twisting.use: # amortisation
        ...
        if motif_or_target and self._cfg.twisting.use: # sampling / guidance
        ...
        if motif_mask is not None and len(motif_mask.shape) == 1:
            motif_mask = motif_mask[None].expand((num_batch, -1))# torch.Size([248])-->torch.Size([1, 248])

        # Set-up time
        if num_timesteps is None:
            num_timesteps = self._sample_cfg.num_timesteps
        min_t = self._cfg.min_t if min_t is None else min_t#partial
        ts = torch.linspace(min_t, 1.0, num_timesteps)
        t_1 = ts[0]
        ...
        for i, t_2 in enumerate(ts[1:]):
            ...
            use_twisting = motif_or_target and self._cfg.twisting.use and t_1 >= self._cfg.twisting.t_min
            ...
            if self._cfg.self_condition:
                if motif_or_target and (motif_groups_mask is None):
                    batch['trans_sc'] = (
            ...
            if motif_or_target and (not self._cfg.twisting.use) and (motif_groups_mask is None):
