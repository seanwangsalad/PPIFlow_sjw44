...
from core.utils.rigid_utils import Rigid
...
class TriangleMultiplication(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, equation='bikc,bjkc->bijc', num_intermediate_channel=128):
        """
        equation: str
        ...
        assert equation == 'bikc,bjkc->bijc'
        ...
        self.center_layer_norm = nn.LayerNorm(num_intermediate_channel)
        ...
        self.left_gate = Linear(input_dim, num_intermediate_channel)
        self.right_gate = Linear(input_dim, num_intermediate_channel)
        ...
        self.gating_linear = Linear(input_dim, output_dim)
        # if zero_init:
        #     torch.nn.init.zeros_(self.output_projection.weights)
        #     torch.nn.init.zeros_(self.gating_linear.weights)


    def forward(self, act, act2, mask):
        """
        act:  [b, N_res, N_pnt, c_z]
        act2:  [b, N_res, N_res, c_z]
        mask: [b, N_res, N_pnt]
        """
        assert act.ndim == 4
        assert mask.ndim == 3
        assert act.shape[3] == self.input_dim

        mask = mask[..., None]  # [b, N_res, N_res, 1]
...
class EdgeTransition(nn.Module):
    def __init__(
        ...
        if use_tri_update:
            self.tri_update_layer = TriangleMultiplication(edge_embed_out, edge_embed_out)

    def forward(self, node_embed, edge_embed, rc_embed=None, rc_mask=None):
        ...
        if rc_embed is not None:
            edge_embed = self.tri_update_layer(rc_embed, edge_embed, rc_mask) + edge_embed
...
class InvariantPointAttention(nn.Module):
    ...
    def forward(
        ...
        Args:
            s:
                [*, N_res, C_s] single representation  (node_embed)
            z:
                [*, N_res, N_res, C_z] pair representation  (edge_embed)
            r:
                [*, N_res] transformation object  (curr_rigids)
            mask:
                [*, N_res] mask   (node_mask)
        Returns:
        ...
        # [*, N_res, H, C_hidden]
        q = q.view(q.shape[:-1] + (self.no_heads, -1))#torch.Size([1, N, 128])-->torch.Size([1, N, 8, 16])

        # [*, N_res, H, 2 * C_hidden]
        kv = kv.view(kv.shape[:-1] + (self.no_heads, -1))#torch.Size([1, N, 256])-->torch.Size([1, N, 8, 32])

        # [*, N_res, H, C_hidden]
        k, v = torch.split(kv, self.c_hidden, dim=-1)#k:torch.Size([1, N, 8, 16]), v:torch.Size([1, N, 8, 16])
        ...
        q_pts = torch.stack(q_pts, dim=-1)#torch.Size([1, 596, 192])-->torch.Size([1, N, 64, 3])
        q_pts = r[..., None].apply(q_pts)

        # [*, N_res, H, P_q, 3]
        q_pts = q_pts.view(
            q_pts.shape[:-2] + (self.no_heads, self.no_qk_points, 3)
        )#torch.Size([1, N, 8, 8, 3])
