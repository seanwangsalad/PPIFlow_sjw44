...
class EdgeFeatureNet(nn.Module):
    ...
    def __init__(self, module_cfg):
        ...
        if self._cfg.embed_diffuse_mask:
            total_edge_feats += 2
        if (self._cfg.task in ["binder", "binder_motif", "binder_rotamer", "binder_motif_partial"]) and self._cfg.embed_hotspot_mask:# partial
            total_edge_feats += 2
        if self._cfg.embed_chain:
            total_edge_feats +=2
    ...
    def forward(self, s, t, sc_t, p_mask, diffuse_mask, hotspot_mask=None, chain_id=None):  #
        ...
        # [b, n_res]
        r = torch.arange(
            num_res, device=s.device).unsqueeze(0).repeat(num_batch, 1)#torch.Size([1, 70])
        relpos_feats = self.embed_relpos(r)#torch.Size([1, 70, 70, 64])

        dist_feats = calc_distogram(
            t, min_bin=1e-3, max_bin=20.0, num_bins=self._cfg.num_bins)#torch.Size([1, 70, 70, 22])
        sc_feats = calc_distogram(
            sc_t, min_bin=1e-3, max_bin=20.0, num_bins=self._cfg.num_bins)#torch.Size([1, 70, 70, 22])
        ...
        edge_feats = self.edge_embedder(torch.concat(all_edge_feats, dim=-1))#old: torch.Size([1, 70, 70, 238])-->torch.Size([1, 70, 70, 128])  new: torch.Size([1, 70, 70, 240])-->torch.Size([1, 70, 70, 128])
