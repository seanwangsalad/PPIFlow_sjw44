...
from analysis.metrics import hotspot_to_binder_center
...
def _merge_pdb_files(
    antigen_pdb: str, antibody_pdb: str, output_pdb: str, antigen_chain: str,
) -> None:
...
def process_file(
    ...
    _merge_pdb_files(
        antigen_file_path, antibody_file_path, merged_filepath, input_info["antigen_chain"]
    )
...
python sample_antibody_nanobody.py \
    --antigen_pdb test_data/8xs3_antigen.pdb \
    --antigen_chain C \
    --specified_hotspots C56,C58 \
    --framework_pdb test_data/8xs3_framework.pdb \
    --heavy_chain A \
    --light_chain B \
    --cdr_length CDRH1,5-12,CDRH2,4-17,CDRH3,7-26,CDRL1,5-12,CDRL2,3-10,CDRL3,4-13 \
    --config ./configs/test_antibody.yaml \
    --samples_per_target 5 \
    --model_weights /path/to/antibody_model.ckpt \
    --output_dir ./output_antibody \
    --name 8xs3

# Nanobody (single-chain):
python sample_antibody_nanobody.py \
    --antigen_pdb test_data/1IJZ_IL13.pdb \
    --framework_pdb test_data/3eak_nanobody_framework.pdb \
    --antigen_chain C \
    --heavy_chain A \
    ...
    --model_weights /path/to/nanobody_model.ckpt \
    --output_dir ./output_nanobody \
    --name nanobody_test
"""
